...
While the [`RadReportExtractor()`](`radreportparser.RadReportExtractor`) class is designed specifically for radiology reports, `radreportparser` also provides a more generic [`extract_section()`](`radreportparser.extract_section`) function that can extract text sections from any document using custom start and end markers.

### Basic Usage

The `extract_section()` function requires:
...
The function's flexibility makes it useful for parsing various medical documents. Here is an example for **pathology report**
...
# Extract microscopic examination
micro = extract_section(
    text=path_text,
...
- Section ends by matching *any* of the `end_keys`
...
- Section ends by matching the `end_keys` sequentially
